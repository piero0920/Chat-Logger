<template>
  ...
  export default {
    ...
    methods:{
      fetchChat(){      
        fetch('https://api.kala-vods.com/chat/KalathrasLolweapon/'+ this.$route.params.user)
        .then(res => res.json())
        .then((data) => {
          const dataChats = [];
          for(var key in data){
      ...
      fetchEmotes(){
        let fetchEmotes = 'https://api.twitch.tv/helix/chat/badges?broadcaster_id=184325380';
