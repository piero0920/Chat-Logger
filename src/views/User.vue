<template>
  <div class="simplebar-scroll-content">
    <div class="simplebar-content" v-if="userMessages.length">
      <div class="totalMessages" title="Total Messages">{{userMessages.length}}</div>
      <div class="message-container" v-for="chat in userMessages" :key="chat._id">
...
<script>
  ...
  import { fetchUserMessages, fetchChannelBadges, fetchGlobalBadges } from '../components/methods'
  export default {
    components: { Github },
    title () {
  ...
  watch: {
    userMessages(newchats, oldchats) {
      if(newchats) {
        this.$nextTick(() => {
          this.$scrollTo('#bottom')
        })
  ...
  async mounted() {
    this.userMessages = await fetchUserMessages(this.$route.params.channel, this.$route.params.user)
